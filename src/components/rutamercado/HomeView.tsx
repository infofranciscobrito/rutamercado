...
export function HomeView({
  ...
  search: HomeSearch;
  /** Ruta reportada en analíticas. */
  trackAs?: string;
  /** id del bloque al que se hace scroll suave al cargar. */
  scrollTo?: string;
}) {
  ...
  // Scroll suave al bloque indicado (reutiliza el mismo id/scroll-margin del ancla).
  useEffect(() => {
    if (!scrollTo) return;
  ...
  const clearFilters = () => {
    void navigate({
      ...
      search: (prev: Record<string, unknown>) =>
        ({ ...defaultFilters, market: prev.market }) as never,
      replace: true,
    });
  };
  ...
  const closeMarket = () => {
    void navigate({
      to: "/",
      search: (prev: Record<string, unknown>) =>
        ({ ...prev, market: undefined }) as never,
      replace: true,
    });
  };

  const switchToWeek = () => updateFilters({ date: "week", day: undefined });
...
function MarketsContent({
  ...
  const municipalities = useMemo(
    () =>
      Array.from(new Set(markets.map((m) => m.municipality))).sort((a, b) =>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-3">
              <p className="text-sm text-[#6B7280]">
                Mostrando{" "}
                <span className="font-semibold text-[#18253f]">
                  {filtered.length}
                </span>{" "}
